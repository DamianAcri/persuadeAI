...
export function DashboardOverview() {
  ...
  // Estados actuales para análisis de conversación
  const [isFileProcessing, setIsFileProcessing] = useState(false);
  const [fileError, setFileError] = useState<string | null>(null);
  
  // Nuevos estados para análisis de scripts
  const [isScriptFileProcessing, setIsScriptFileProcessing] = useState(false);
  ...
  // Nuevos manejadores para la subida de archivos de script
  function handleScriptFileExtracted(extractedText: string) {
    if (extractedText) {
      const encodedText = encodeURIComponent(extractedText);
      router.push(`/ai/script-analysis?script=${encodedText}&method=upload`);
    } else {
      setScriptFileError("No text could be extracted from the file. Please try another file.");
    }
  }

  function handleScriptFileProcessingState(isProcessing: boolean) {
    setIsScriptFileProcessing(isProcessing);
    if (isProcessing) {
        ...
        <TabsContent value="overview" className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <FileUploadHandler 
                  onTextExtracted={handleScriptFileExtracted} 
                  onProcessingStateChange={handleScriptFileProcessingState}
                />
                {isScriptFileProcessing && (
                  <div className="mt-2 text-sm text-blue-600 flex items-center">
                    <svg className="animate-spin -ml-1 mr-2 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>

...
const getSessionFromCookies = () => {
  ...
  try {
    ...
    if (!authCookieStr) {
      console.log('🍪 [scriptAnalysisService] No se encontró cookie de autenticación');
      return null;
    }
    
    console.log('🍪 [scriptAnalysisService] Cookie de autenticación encontrada');
    ...
    try {
      ...
      console.error('❌ [scriptAnalysisService] Error al analizar cookie:', parseError);
    }
    
    return null;
  } catch (error) {
    console.error('❌ [scriptAnalysisService] Error al leer cookies:', error);
    return null;
  }
};

export async function analyzeScript({ script, targetAudience, productType }: ScriptAnalysisParams) {
  try {
    console.log('🔍 [scriptAnalysisService] Verificando sesión antes de análisis...');
    ...
    console.log(`🔐 [scriptAnalysisService] Estado final: ${isAuthenticated ? 'Autenticado ✅' : 'No autenticado ❌'}`);
    if (finalUserId) {
      console.log(`👤 [scriptAnalysisService] Usuario: ${finalUserId}`);
    }
    ...
    if (cookieSession?.access_token) {
      ...
      console.log('🔒 [scriptAnalysisService] Utilizando token de cookie para autenticación');
      ...
      try {
        ...
        if (sessionData.session?.access_token) {
          ...
          console.log('🔒 [scriptAnalysisService] Utilizando token de Supabase para autenticación');
        }
      } catch (authError) {
        console.error('❌ [scriptAnalysisService] Error al obtener token:', authError);
      }
    }

    // Hacer la solicitud a la API
    console.log('📤 [scriptAnalysisService] Enviando solicitud de análisis de script...');
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error || errorData.message || 'Error en la solicitud de análisis de script');
    }

    // Procesar resultado
    const result = await response.json();
    
    console.log('📥 [scriptAnalysisService] Respuesta recibida');
    
    return result;
  } catch (error: any) {
    console.error('❌ [scriptAnalysisService] Error en análisis:', error);
    throw new Error(`Error al analizar script de ventas: ${error.message}`);
  }
}

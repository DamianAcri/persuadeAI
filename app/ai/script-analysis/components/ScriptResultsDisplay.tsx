...
export default function ScriptResultsDisplay({
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        console.log(
          "🔍 [ScriptResultsDisplay] Sesión al cargar:",
          data.session ? "Activa" : "Inactiva"
        );
        ...
        if (data.session) {
          ...
          // Verify that result belongs to current user
          if (result.userId && result.userId !== data.session.user.id) {
            console.error("🚫 [ScriptResultsDisplay] User ID mismatch!");
            setErrorMessage("You are not authorized to view this analysis");
            setSaveStatus("error");
          }
        } else {
  ...
  const handleSaveResult = async () => {
    try {
      ...
      // Verify user matches the analysis user ID
      if (result.userId && result.userId !== userId) {
        setErrorMessage("You can only save your own analysis results");
        setSaveStatus("error");
        return;
      }
        ...
        <div className="w-full bg-gray-200 rounded-full h-3">
          <div
            className={`h-3 rounded-full ${
              value >= 8 ? "bg-green-600" : value >= 6 ? "bg-blue-600" : value >= 4 ? "bg-yellow-600" : "bg-red-600"
            }`}
    ...
    <div className="mt-8 border rounded-lg overflow-hidden">
          ...
          <div className="flex items-center">
            {!user && (
              <div className="flex items-center mr-3">
                <span className="text-yellow-600 text-sm mr-2">Sign in to save results</span>
                <button onClick={handleLoginRedirect} className="text-sm underline text-blue-600">
                  Login
                </button>
              </div>
            )}
            ...
            {saveStatus === "success" && (
              <span className="text-green-600 mr-3 text-sm">Analysis saved successfully!</span>
            )}

            {saveStatus === "error" && errorMessage && (
              <span className="text-red-600 mr-3 text-sm">{errorMessage}</span>
            )}
            ...
            <button
              ...
              className={`px-4 py-2 rounded-md text-white font-medium ${
                isSaving || !user ? "bg-gray-400 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"
              }`}
            >
      ...
      {debugInfo && (
        <div className="bg-gray-100 border-b border-gray-200 p-2 text-xs">
          <details>
            ...
            <pre className="p-2 text-gray-700 overflow-x-auto">{debugInfo}</pre>
          </details>
        </div>
      )}
      ...
      <div className="p-6 border-b bg-white">
        <div className="flex flex-col md:flex-row md:items-center">
          ...
          <div className="flex flex-col items-center p-4 bg-gray-50 rounded-lg">
            <span className="text-sm font-medium text-gray-500 mb-1">Persuasiveness Score</span>
            <span className={`text-4xl font-bold ${getScoreColor(result.persuasiveness_score)}`}>
      ...
      <div className="bg-gray-50 border-b">
        <nav className="flex flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-3 text-sm font-medium ${
              activeTab === "strengths" ? "border-b-2 border-green-500 text-green-600" : "text-gray-500 hover:text-gray-700"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-3 text-sm font-medium ${
              activeTab === "weaknesses" ? "border-b-2 border-red-500 text-red-600" : "text-gray-500 hover:text-gray-700"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-3 text-sm font-medium ${
              activeTab === "suggestions" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500 hover:text-gray-700"
            }`}
          >
      ...
      <div className="p-6 bg-white">
        {activeTab === "overview" && (
          <div>
            <div className="mb-6">
              <h4 className="font-medium text-gray-700 mb-2">Persuasiveness Score Breakdown</h4>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-blue-50 border border-blue-100 rounded-lg">
                  <h5 className="font-medium text-blue-800 mb-2">Top Suggestions</h5>
                  <ul className="list-disc pl-5 text-sm text-gray-700 space-y-1">
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-gray-600 mb-2">
                  Measures how easily your audience can understand your message. Clear scripts use simple language, avoid
                  jargon, and present ideas logically.
                </p>
                {renderMetricGauge("Clarity", result.clarity_score)}
              </div>
              ...
              <div>
                ...
                <p className="text-gray-600 mb-2">
                  Evaluates how well your script flows from beginning to end. Well-structured scripts have a clear
              ...
              <div>
                ...
                <p className="text-gray-600 mb-2">
                  Measures how well your script emphasizes benefits rather than features. Effective scripts connect
                  product attributes to audience needs.
                </p>
                {renderMetricGauge("Benefit Focus", result.benefit_focus_score)}
              </div>
              ...
              <div>
                ...
                <p className="text-gray-600 mb-2">
                  Evaluates how genuine and trustworthy your script sounds. Authentic scripts avoid hyperbole and build
                  credibility.
                </p>
                {renderMetricGauge("Authenticity", result.authenticity_score)}
              </div>
              ...
              <div>
                <h5 className="font-medium text-gray-700 mb-2">
                  Call to Action Effectiveness: {result.call_to_action_effectiveness}/10
                </h5>
                <p className="text-gray-600 mb-2">
                  Measures how effectively your script motivates the audience to take action. Strong CTAs are clear,
                  specific, and create a sense of urgency.
                </p>
                {renderMetricGauge("Call to Action", result.call_to_action_effectiveness)}
              </div>
            </div>
